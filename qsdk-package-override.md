...
- `BUILDONLY:=1` 在 12.5 真正生效（`scripts/package-metadata.pl` 5 处过滤），把 stock 整个剔除 kconfig + 依赖图，stock `.ipk` 不产
...
| 漏写 `PROVIDES`（仅靠 BUILDONLY 抑制 stock） | 12.5 `package-metadata.pl` 真过滤 BUILDONLY，stock `.ipk` 没产 → 所有 `+stock` 依赖 unresolved |
...
| `BUILDONLY:=1` | ✅ | 隐藏 kconfig + 把 stock 整个剔除 kconfig/依赖图（12.5 `package-metadata.pl` 5 处过滤） |
